...
class DriverBasedTests {
    private val nodeNames = listOf(DUMMY_NOTARY.name, DUMMY_BANK_A.name, DUMMY_BANK_B.name)
    ...
    fun `node webserver test`() {
        driver(isDebug = true, startNodesInProcess = true) {
            ...
            nodeHandles.forEachIndexed { idx, nodeHandle ->
                startWebserver(nodeHandle).getOrThrow()

                val nodeAddress = nodeHandle.webAddress
                val url = "http://$nodeAddress/api/example/me"

                val request = Request.Builder().url(url).build()
                val client = OkHttpClient()
                val response = client.newCall(request).execute()

                assert(response.body().string().contains(nodeNames[idx].toString()))
            }
        }
    }
}